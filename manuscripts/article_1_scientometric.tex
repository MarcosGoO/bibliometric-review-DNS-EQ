...
Country-level analysis (degree $\geq$ 2) revealed strong international collaboration (Figure \ref{fig:countrynet}). The United States (35 documents, 10.9\%) emerged as the most productive country, followed by Germany (29, 9.0\%) and Turkey (19, 5.9\%). Centrality analysis identified the United States, Germany, and China as bridging nodes facilitating cross-regional knowledge exchange.

Network density at the country level (0.24) substantially exceeded author-level density, indicating that international collaboration is more prevalent than individual-level partnerships. This pattern suggests institutional agreements and funding mechanisms enable cross-border projects despite limited sustained individual collaborations.

\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../outputs/figuras/redes/02_country_collaboration_network.png}
\caption{Country collaboration network (degree $\geq$ 2). Node size reflects publication count; edges represent co-authorship links.}
\label{fig:countrynet}
\end{figure}
