...
Software implementation utilized R version 4.3.2 with \texttt{biblio\-metrix} version 4.1 \cite{Aria2017} for bibliometric analysis and \texttt{igraph} version 2.0 \cite{Csardi2006} for network operations. All visualizations employed \texttt{ggplot2} and \texttt{ggraph} for publication-quality graphics.
...
\textbf{Country Collaboration Network.} Country-level analysis followed identical procedures using \texttt{biblio\-Network(\allowbreak network=\allowbreak"countries")}, filtered to countries with degree $\geq$ 2. This revealed international research partnerships and geographic centrality.

\textbf{Technical Note.} All \texttt{igraph} functions employed explicit namespace qualification (\texttt{igraph::\allowbreak degree()}, \texttt{igraph::\allowbreak between\-ness()}, \texttt{igraph::\allowbreak close\-ness()}, \texttt{igraph::\allowbreak cluster\_\allowbreak louvain()}, \texttt{igraph::\allowbreak member\-ship()}) to avoid namespace conflicts with \texttt{tidy\-verse} packages. Following \texttt{induced\_\allowbreak sub\-graph()} operations, vertex attributes were recalculated on the subgraph to ensure metric accuracy.

\subsubsection{Conceptual Structure}

Keyword co-occurrence networks captured thematic relationships using \texttt{biblio\-Network(\allowbreak analysis=\allowbreak"co-\allowbreak occur\-rences",\allowbreak network=\allowbreak"key\-words")}. Filtering retained keywords with degree $\geq$ 3 to focus on central concepts. Community detection identified thematic clusters, while centrality metrics revealed bridging concepts connecting multiple domains.

\section{Results}

\subsection{Publication Trends and Landscape}

The field exhibited exponential growth, particularly post-2015, with publications increasing from 14 in 2016 to 52 in 2025 (Figure \ref{fig:production}). This acceleration coincides with international STEM education policy initiatives and increased research funding. Document type analysis (Figure \ref{fig:doctypes}) revealed 94\% research articles and 6\% reviews, indicating an empirically driven field with emerging synthetic scholarship.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.8\textwidth]{figuras/01_produccion_anual.png}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.7\textwidth]{figuras/02_tipos_documento.png}
...
The middle zone (Zone 2, 40 journals) and peripheral zone (Zone 3, 104 journals) accounted for 67\% of publications, indicating broad disciplinary engagement beyond the concentrated core. This distribution aligns with theoretical expectations for mature scientific fields. Figure \ref{fig:bradford} visualizes the three-zone structure.

\begin{figure}[!ht]
\centering
\includegraphics[width=0.85\textwidth]{figuras/03_bradford_law.png}
\caption{Bradford's Law zones showing article distribution across core, middle, and peripheral journals.}
\label{fig:bradford}
\end{figure}

\FloatBarrier
\subsection{Author Productivity and Lotka's Law}
Lotka's Law analysis revealed an extreme concentration of productivity within the dataset, as detailed in Table \ref{tab:lotka}. Of the 1,247 unique authors identified, a substantial 89.1\% contributed exactly one document, while only 8.7\% published two. Authors with three or more publications—the "prolific core"—constituted a mere 2.2\% of the total population. This distribution exceeds typical Lotka coefficients ($n \approx 2$), indicating a higher degree of output inequality than is usually observed in established scientific disciplines. Such a stark imbalance suggests that the field of natural sciences education research is heavily characterized by transient contributions—potentially from doctoral students or practitioners conducting one-off studies; rather than being driven by long-term, sustained research programs.
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.95\textwidth]{figuras/01_author_collaboration_network.png}
\caption{Author collaboration network (degree $\geq$ 3). Node size represents degree centrality; colors indicate community membership detected via Louvain algorithm.}
\label{fig:authornet}
\end{figure}
...
\begin{figure}[!ht]
\centering
\includegraphics[width=0.95\textwidth]{figuras/07_conceptual_structure_keywords.png}
\caption{Keyword co-occurrence network (degree $\geq$ 3). Node size indicates keyword frequency; colors represent thematic communities identified via Louvain algorithm.}
\label{fig:keywords}
\end{figure}
